...
function MoveCell({ value, legal, highlight, corrected, active, reason, confidence, disputed, onEdit, onShowBoard }: {
  ...
  const bg = active ? 'bg-blue-600/40 text-blue-100' : corrected ? 'bg-green-900/50 text-green-200' : highlight ? 'bg-red-900/50 text-red-200' : isLowConfidence ? 'bg-red-500/20 text-red-200' : isIllegal ? 'bg-orange-500/25 text-orange-200' : isMediumConfidence ? 'bg-yellow-500/20 text-yellow-100' : 'text-slate-100';
