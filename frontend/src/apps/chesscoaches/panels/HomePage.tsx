...
function OnboardingBanner({ status }: { status: OnboardingStatus }) {
  ...
  const steps = [
    { done: status.has_profile, before: t('coaches.onboarding.step.profile.before.coach'), link: t('coaches.onboarding.step.profile.link.coach'), after: '', path: '/chess/app/profile' },
    { done: status.has_students, before: t('coaches.onboarding.step.students.before'), link: t('coaches.onboarding.step.students.link'), after: '', path: '/chess/app/students' },
    { done: status.has_lessons, before: t('coaches.onboarding.step.calendar.before'), link: t('coaches.onboarding.step.calendar.link'), after: '', path: '/chess/app/schedule' },
  ];
  ...
  return (
    <div className={`${animateClasses} mt-2 flex flex-col min-h-[calc(100dvh-80px)]`}>
      ...
      <div className="max-w-4xl w-full mx-auto px-[5%] space-y-4">
        {NAV_SECTIONS.map(({ titleKey, items }) => {
          ...
          const highlightPath = onboarding && !onboarding.has_lessons
            ? (!onboarding.has_profile ? '/chess/app/profile' : !onboarding.has_students ? '/chess/app/students' : '/chess/app/schedule')
            : null;
          return (
      ...
      <div className="text-center pt-8 pb-6">
        <button
          onClick={() => navigate('/chess/app/about')}
