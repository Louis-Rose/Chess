...
const CARDS: { id: string; path: string; icon: LucideIcon; hoverBorder: string; iconBg: string; titleKey: string; descriptionKey: string | null }[] = [
  ...
  {
    id: 'when-to-play',
    path: '/chess/when-to-play',
    icon: Clock,
    hoverBorder: 'hover:border-amber-500',
    iconBg: 'bg-amber-600',
    titleKey: 'chess.whenToPlayTitle',
    descriptionKey: null,
  },
];
  ...
  if (!description) {
    return (
      <>
        ...
        <h3 className="text-lg font-bold text-slate-100 select-text text-center text-balance px-12 py-4">{title}</h3>
