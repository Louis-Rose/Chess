...
export function QuarterlyResults({ portfolioTickers }: QuarterlyResultsProps) {
  ...
  return (
    <div>
      ...
      {report ? (
        <div ref={contentRef}>
          {/* Title - shown only during download */}
          {isDownloading && (
          ...
          <div className="mx-[10%]">
                  ...
                  <>{/* Section header */}
                    ...
                    {section.rows.map((row, rIdx) => {
                      ...
                      return (
                        ...
                        >
                          <td className={`py-2 px-3 border-r-2 border-slate-400 dark:border-slate-300 text-left text-slate-800 dark:text-slate-100 w-[50%] ${
            ...
            {/* Conference call insights - bordered box */}
            {report.insights && report.insights.length > 0 && (
              <div className="mt-6 border-2 border-slate-400 dark:border-slate-300 rounded-lg p-4">
          ...
          {/* LUMNA branding - visible on page */}
          {!isDownloading && (
      ...
      {/* Download button - below content, hidden during download */}
      {report && !isDownloading && (
        <div className="flex justify-end mt-2 mr-2">
          <button
            onClick={handleDownload}
            className="p-2 rounded-lg text-slate-500 hover:text-slate-700 dark:text-slate-400 dark:hover:text-slate-200 hover:bg-slate-200 dark:hover:bg-slate-600 transition-colors"
