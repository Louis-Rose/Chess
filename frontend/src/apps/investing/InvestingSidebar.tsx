...
export function InvestingSidebar() {
  ...
  return (
    <div className="w-64 bg-white dark:bg-slate-900 h-screen p-4 flex flex-col gap-2 sticky top-0 overflow-y-auto">
      {/* LUMRA Logo */}
      <Link
        to="/investing"
        className="flex items-center justify-center gap-3 px-2 pb-4 mb-2 border-b border-slate-200 dark:border-slate-700 hover:opacity-80 transition-opacity"
      >
        <div className="w-10 h-10 bg-green-600 rounded-lg flex items-center justify-center">
          <BarChart3 className="w-6 h-6 text-white" />
        </div>
        <span className="text-xl font-bold text-slate-900 dark:text-white tracking-wide">LUMRA</span>
      </Link>

      {/* User Menu */}
      <div className="flex justify-center mb-4 px-2 pb-4 border-b border-slate-200 dark:border-slate-700">
        {isAuthenticated ? (
          authLoading ? (
          ...
          <Link to="/" className="flex items-center gap-2 text-slate-500 hover:text-slate-700 dark:hover:text-slate-300 transition-colors">
      ...
      */}
      ...
      {/* Navigation */}
      <div className="flex flex-col gap-1 px-2 py-4 border-b border-slate-200 dark:border-slate-700">
        {navItems.map((item) => (
          <NavLink
            key={item.path}
            to={item.path}
      ...
      <div className="mt-auto px-2 pt-4 border-t border-slate-200 dark:border-slate-700">
