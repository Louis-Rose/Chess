...
// Auth gate for the fit app: shown when no one is logged in. Uses the shared
// lumna.co login, so signing in here (or anywhere on Lumna) covers the gym.
// redirectTo="" keeps the user on /fit after login.
export function FitLogin() {
  return (
    <div className="flex min-h-dvh flex-col items-center justify-center gap-6 bg-slate-900 px-8 text-center text-slate-100">
      ...
      <LoginButton size="large" redirectTo="" />
    </div>
  );
}
